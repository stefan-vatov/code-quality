...
The config errors on lines over 150 characters, files over 500 lines, functions over 75 lines, nesting deeper than 3 levels, more than 5 parameters, cyclomatic complexity over 10, denies unsafe `any` escape hatches (no-unsafe-call, no-unsafe-member-access, plus no-unsafe-assignment/return/argument when type-aware), and denies unhandled promises (no-floating-promises, no-misused-promises when type-aware).
