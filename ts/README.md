...
- Effect-aware out of the box: 102 always-on Effect rules target lazy values, generator style, Promise boundaries, typed errors, Schema boundaries, resources, tests, and common hallucinated APIs.
...
`effect-prefer-ref-getAndUpdate` is always on, import-aware, and report-only for Effect v3.21.2 and v4 beta.101. It reports exact data-first or curried `Ref.modify` calls whose concise, untyped, one-parameter arrow returns `[current, next]`, optionally parenthesized or `as const`, and recommends `Ref.getAndUpdate`. It excludes no-op `[current, current]` callbacks, typed or widened tuples, block or classic-function callbacks, wrappers, wrong provenance or shadowing, and unsupported calls. The shared operation preserves atomicity, laziness, evaluation, and return type; the v4 direct path also avoids allocating the tuple.

- Effect v3.21.2 `Ref.getAndUpdate` implementation ([source](https://github.com/Effect-TS/effect/blob/39c934c1476be389f7469433910fdf30fc4dad82/packages/effect/src/internal/ref.ts#L64-L67))
- Effect v4 beta.101 `Ref.getAndUpdate` and `Ref.modify` APIs ([getAndUpdate](https://github.com/Effect-TS/effect/blob/ed2afb3424e90f3b98a6e4740f4e12cc08e3cc11/packages/effect/src/Ref.ts#L323-L331), [modify](https://github.com/Effect-TS/effect/blob/ed2afb3424e90f3b98a6e4740f4e12cc08e3cc11/packages/effect/src/Ref.ts#L490-L498))
- Effect Native CDP state update ([source](https://github.com/effect-native/effect-native/blob/df994cc632071e80ab78280400573586258aed3e/packages/debug/src/internal/Cdp.ts#L260))
- T3 Code desktop preview manager ([source](https://github.com/pingdotgg/t3code/blob/b41e89eba9cd232cc3257b400fc30972a9b53438/apps/desktop/src/preview/Manager.ts#L437-L438))
- T3 Code Codex app server protocol ([source](https://github.com/pingdotgg/t3code/blob/b41e89eba9cd232cc3257b400fc30972a9b53438/packages/effect-codex-app-server/src/protocol.ts#L390-L393))
- T3 Code ACP protocol ([source](https://github.com/pingdotgg/t3code/blob/b41e89eba9cd232cc3257b400fc30972a9b53438/packages/effect-acp/src/protocol.ts#L530-L533))

`effect-prefer-yieldable-error-over-fail` is always on, import-aware, and report-only for Effect v3.21.2 and v4 beta.101. It reports only a directly delegated `yield* Effect.fail(new Error)` in an exact inline synchronous `Effect.gen`, `Effect.fn`, or `Effect.fnUntraced` generator, where `Error` is an unchanged top-level, empty, undecorated class directly extending imported `Data.Error`, `Data.TaggedError` with a literal tag, or an exact `Schema.TaggedError` factory construction. Those official constructors produce `Cause.YieldableError` instances, so yielding them directly preserves the lazy typed failure and its error-channel type. The rule excludes wrapped or unsupported calls, non-delegated yields, non-inline hosts, shadowing, class mutation, and any ineligible constructor. This conservative boundary matches the [official dual-version diagnostic](https://github.com/Effect-TS/language-service/blob/6b9dab53ff1d02270358f09ecb3290d0f7d244/packages/language-service/src/diagnostics/unnecessaryFailYieldableError.ts#L9-L84).

- Effect v4 bundle reporter ([source](https://github.com/Effect-TS/effect/blob/ed2afb3424e90f3b98a6e4740f4e12cc08e3cc11/packages/tools/bundle/src/Reporter.ts#L247-L255))
- Distilled Cloudflare auth ([source](https://github.com/alchemy-run/distilled-cloudflare/blob/8ebd4225485be321c0bdfe8649a9f03d17e11425/src/auth.ts#L238-L246))
- Effect Native LibSQL effect ([source](https://github.com/effect-native/effect-native/blob/df994cc632071e80ab78280400573586258aed3e/packages/libcrsql/src/effect.ts#L98-L102))
- Typed platform service ([source](https://github.com/TylorS/typed/blob/3b44be752873fb43497539783e47ffc642411182/packages/core/src/Platform.ts#L189-L195))
- AnswerOverflow auto-thread service ([source](https://github.com/AnswerOverflow/AnswerOverflow/blob/f2f443f3c01a67d0e98dbdd1443f9519dd21251b/apps/discord-bot/src/services/auto-thread.ts#L20-L34))

`effect-schema-no-redundant-tag-identifier` is always on, import-aware, and report-only for Effect v3.21.2 and v4 beta.101. It reports a class declaration only when its direct superclass is an official Schema tagged-class factory whose inner call has exactly one non-spread string-literal identifier and whose outer call has exactly a string-literal `_tag`, fields, and optional annotations; the identifier and tag must have equal decoded values. The supported APIs are v3 `TaggedClass`, `TaggedError`, and `TaggedRequest`, plus v4 `TaggedClass` and `TaggedErrorClass`. Both versions derive the identifier as `identifier ?? tag`, so omitting the duplicate preserves the schema identifier. The matcher accepts official root and `effect/Schema` imports and aliases, including root-package namespaces, while rejecting foreign or type-only imports, lexical shadowing, computed or optional calls, wrappers, spreads, unsupported arities, class expressions, explicit outer generics, and non-literal or unequal values. This conservative contract follows Effect's [official dual-version diagnostic](https://github.com/Effect-TS/language-service/blob/6b9dab53ff1d02270358f09ecb3290d0b0f7d244/packages/language-service/src/diagnostics/redundantSchemaTagIdentifier.ts#L8-L68) and [v4 Schema implementation](https://github.com/Effect-TS/effect/blob/ed2afb3424e90f3b98a6e4740f4e12cc08e3cc11/packages/effect/src/Schema.ts#L12863-L13032). Mined duplicate identifiers include:

- Hazel Electric proxy bot authentication ([source](https://github.com/HazelChat/hazel/blob/f033d6058021f0cac6a4e461c902122eab32ed91/apps/electric-proxy/src/auth/bot-auth.ts#L21-L26))
- Hazel domain errors ([source](https://github.com/HazelChat/hazel/blob/f033d6058021f0cac6a4e461c902122eab32ed91/packages/domain/src/errors.ts#L4-L15))
- Hazel webhook schemas ([source](https://github.com/HazelChat/hazel/blob/f033d6058021f0cac6a4e461c902122eab32ed91/packages/domain/src/http/webhooks.ts#L11-L19))
...
`effect-prefer-option-nullish-getters` is always on, import-aware, and report-only for both Effect v3 and Effect v4. It reports the exact manual extraction `Option.isSome(option) ? option.value : null` or `undefined`, recommending `Option.getOrNull(option)` or `Option.getOrUndefined(option)`. To avoid changing evaluation order or observable getter behavior, the option must be a simple identifier bound by an initialized, preceding local `const` in the same execution context and switch case. The matcher recognizes root and `effect/Option` aliases, respects lexical shadowing, and excludes parameters, mutable or imported bindings, members, calls, wrappers, optional or computed access, explicit generics, mismatched identifiers, shadowed `undefined`, and alternate expressions. The conservative target appears at three production sites in two Effect v4 repositories, backed by the same canonical getter idiom in Effect v3 codebases and the official v4 API:

- Hazel gateway decode in `discord-gateway-service.ts` ([source](https://github.com/HazelChat/hazel/blob/f033d6058021f0cac6a4e461c902122eab32ed91/apps/backend/src/services/chat-sync/discord-gateway-service.ts#L186))
- Hazel shared gateway decode in `discord-gateway-shared.ts` ([source](https://github.com/HazelChat/hazel/blob/f033d6058021f0cac6a4e461c902122eab32ed91/apps/backend/src/services/chat-sync/discord-gateway-shared.ts#L180))
...
`effect-prefer-option-getOrElse` is always on, import-aware, and report-only for both Effect v3.21.2 and Effect v4 beta.101. It reports the exact data-first `Option.match(option, { onNone: () => fallback, onSome: value => value })` identity-onSome specialization and recommends `Option.getOrElse(option, () => fallback)`. The matcher recognizes official root and `effect/Option` imports and aliases, respects lexical shadowing, and rejects wrappers, computed or optional calls, explicit generics, spreads, extra properties, non-concise handlers, typed or async arrows, and non-identity `onSome` branches. Curried `Option.match({ ... })(option)` is deliberately excluded because replacing it can change contextual type inference in Effect v3. The mined production targets are:

- Hazel bot SDK command option fallback ([source](https://github.com/HazelChat/hazel/blob/f033d6058021f0cac6a4e461c902122eab32ed91/libs/bot-sdk/src/hazel-bot-sdk.ts#L283-L287))
- Hazel chat-sync worker option fallback ([source](https://github.com/HazelChat/hazel/blob/f033d6058021f0cac6a4e461c902122eab32ed91/apps/backend/src/services/chat-sync/chat-sync-core-worker.ts#L2045-L2049))
- Effect-TS tsgo target prompt ([source](https://github.com/Effect-TS/tsgo/blob/bd059259948a49a1c11c2ca3274579ac08049734/_packages/tsgo/src/setup/target-prompt.ts#L79-L82))
- Effect-TS tsgo config loading ([source](https://github.com/Effect-TS/tsgo/blob/bd059259948a49a1c11c2ca3274579ac08049734/_packages/tsgo/src/config.ts#L23-L26))
- T3 Code provider-maintenance scheduling ([source](https://github.com/pingdotgg/t3code/blob/b41e89eba9cd232cc3257b400fc30972a9b53438/apps/server/src/provider/providerMaintenanceRunner.ts#L132-L149))

`effect-prefer-option-orElseSome` is always on, import-aware, and report-only for both Effect v3.21.2 and Effect v4 beta.101. It reports exact data-first or curried `Option.orElse` fallbacks whose concise zero-argument arrow returns one `Option.some(value)`, recommending `Option.orElseSome` and a callback that returns `value` directly. The matcher recognizes official root and `effect/Option` namespaces and named aliases, supports mixed official import styles, respects lexical shadowing, and rejects foreign or type-only imports, invalid direct root-package calls, wrappers, computed or optional calls, explicit generics, spreads, wrong arities, and typed, generic, async, block-bodied, or parameterized callbacks. Both supported Effect versions expose the same `Option<A | B>` overloads and lazy fallback behavior:

- Effect v3.21.2 `Option.orElse` and `Option.orElseSome` ([source](https://github.com/Effect-TS/effect/blob/39c934c1476be389f7469433910fdf30fc4dad82/packages/effect/src/Option.ts#L693))
- Effect v4 beta.101 `Option.orElse` and `Option.orElseSome` ([source](https://github.com/Effect-TS/effect/blob/ed2afb3424e90f3b98a6e4740f4e12cc08e3cc11/packages/effect/src/Option.ts#L657))
- Effect language-service import-property fallback ([source](https://github.com/Effect-TS/language-service/blob/6b9dab53ff1d02270358f09ecb3290d0b0f7d244/packages/language-service/src/core/TypeScriptUtils.ts#L502-L504))
- Effect docgen description and category fallbacks ([source](https://github.com/Effect-TS/docgen/blob/bcc9c14f91c8466ee611bddb36f9e05ed7932166/src/Parser.ts#L521-L532))
- Effect tsgo TypeScript-version fallback ([source](https://github.com/Effect-TS/tsgo/blob/bd059259948a49a1c11c2ca3274579ac08049734/_packages/tsgo/src/setup/target-prompt.ts#L160-L166))
