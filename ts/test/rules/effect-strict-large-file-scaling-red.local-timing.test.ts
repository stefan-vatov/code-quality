...
type RetryRuleBridgeInput = (typeof plugin.rules)[typeof RETRY_RULE] | SourceRule;
...
const sourceRule = (rule: RetryRuleBridgeInput): SourceRule => {
...
const makeFixture = (lineCount: number, sample: number): SparseFixture => {
  ...
  return { ast: programNode(parsed.program), filename, source };
};

const runOneShot = (fixture: SparseFixture): TimedRun => {
  const reports: object[] = [];
  const rule = sourceRule(plugin.rules[RETRY_RULE]);
