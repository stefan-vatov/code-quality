...
const programNode = (program: ProgramBridgeInput): ASTNode => {
  // SAFETY: the parser-valid TSX fixture contains ESTree nodes and primitive metadata
  // accepted by the Effect AST contract; the assertion preserves the original tree.
  return program as ASTNode;
};

const sourceRule = (rule: RetryRuleBridgeInput): SourceRule => {
  // SAFETY: this retry rule is generated by makeRules with the source-context API;
  // eslintCompatPlugin retains its createOnce factory while widening the public types.
