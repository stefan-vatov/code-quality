...
const SERVICE_CONSTRUCTOR_NAME = /^make[A-Z][A-Za-z0-9]*Service$/u;
...
export const noServiceConstructorImportsRule = defineRule({
  meta: {
    ...
    docs: {
      description:
        'Disallow project-local make<CapabilityName>Service imports outside test and spec files.',
    },
    messages: {
  ...
  create(context) {
    ...
    return {
      ImportDeclaration(node) {
        if (isTestFile || node.importKind === 'type' || !isProjectLocalImport(node.source.value)) {
          return;
        }
