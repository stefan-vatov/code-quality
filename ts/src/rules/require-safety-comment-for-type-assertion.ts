...
function isSafetyCommentOptions(value: unknown): value is SafetyCommentOptions {
  return Predicate.isObject(value) && !Array.isArray(value) && 'markers' in value;
}

function configuredSafetyMarkers(option: SafetyCommentOptions | undefined): readonly string[] {
  if (option === undefined) {
  ...
  const markers = configured.flatMap((marker) =>
    Predicate.isString(marker) && marker.trim().length > 0 ? [marker.trim()] : [],
  );
...
export const requireSafetyCommentForTypeAssertionRule = defineRule({
  ...
  createOnce(context) {
    ...
    const checkAssertion = (node: TypeAssertion) => {
      ...
      const markers = configuredSafetyMarkers(isSafetyCommentOptions(option) ? option : undefined);
